...
let country_data = data.counties;
country_data.forEach(function(item,index){
...
country_select.addEventListener('change',check_country,false);

function check_country(e){
    country_dataset = (e.target[e.target.selectedIndex].dataset.index);
    let districts_data = data.districts;
    if(e.target.value == '請選擇縣市'){
        let str = `<option value="請選擇鄉鎮">請選擇鄉鎮</option>`;
        district_select.innerHTML = str;
    }else{
        districts_data.forEach(function(item,index){
            if(country_dataset == index){
                let content = "";
                districts_data[index].forEach(function(element){
                    let str = `<option value="${element}">${element}</option>`
                    content += str;
                });
                district_select.innerHTML = content;
            };
        });
    }
};